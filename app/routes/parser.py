...
def parse_audio():
    try:
        ...
        if "audio_file" not in request.files:
            return (
                jsonify({"message": "Aucun fichier audio fourni", "status": "error"}),
                400,
            )
        ...
        if file.filename == "":
            return (
                jsonify({"message": "Aucun fichier sélectionné", "status": "error"}),
                400,
            )
        ...
        if not api_key:
            return (
                jsonify({"message": "Clé API OpenAI manquante", "status": "error"}),
                500,
            )
